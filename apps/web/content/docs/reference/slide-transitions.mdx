...
- ❌ Six different transitions across six pages — the loudest "made in
  PowerPoint" tell.
- ❌ `translateX(100%)` slide-from-side — reads as iOS modal or PowerPoint
  Push, not a slide change.
- ❌ Aggressive scale-pop (e.g. `0.85 → 1`) + blur — lightbox / photo-viewer
  vocabulary; implies zooming *into* something.
...
- ❌ Declaring a transition on every deck. If you don't have a clear reason,
  omit it. Snap-swap is fine.
